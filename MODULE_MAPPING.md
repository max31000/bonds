...
- T-Invest токен через UI (модуль 9) — хранится только в БД на аккаунт (без ENV-фолбэка); коннектор (модуль 1) переключён на чтение из этого хранилища через `ITInvestTokenProvider` — см. Решение 12 в `BACKEND_DECISIONS.md`.
