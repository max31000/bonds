...
**Этапы 01–03 выполнены** (фундамент/CI-конфигурация, Telegram-авторизация, доменная
модель и хранилище). Этап 04 (внешние коннекторы T-Invest/MOEX) — следующий.
...
- **Этап 01** — скаффолдинг монорепо, CI-воркфлоу (`backend.yml`/`frontend.yml`),
  ...
  **не выполнялся** — см. раздел "Деплой — TODO" ниже (он остаётся актуальным и после
  этапов 02–03: ни один из них не требовал живого VDS/GitHub-репозитория).
...
Доменная математика (НКД, YTM, дюрации, G-спред), коннекторы MOEX/T-Invest, движок
сигналов и собственно фронтовые дашборды — этапы 04+, ещё не реализованы.
...
## MVP-ограничения (после этапа 03)

- Коннекторов к T-Invest/MOEX нет — справочник и хранилище готовы, но не наполняются
  автоматически (этап 04). Расчётный движок (НКД, YTM, дюрации, G-спред) не реализован
  (этап 05), как и cash-flow проекция/сигналы/планировщик (этапы 06–07) и доменные
  HTTP-эндпоинты/фронтовые дашборды сверх логина (этапы 08–09).
- Деплой на живой VDS не выполнялся ни на одном из этапов 01–03 — см. "Деплой — TODO".
