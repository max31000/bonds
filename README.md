...
**Этап 01 (фундамент: скаффолдинг + CI/CD-конфигурация) выполнен.**
Доменной логики пока нет — только инфраструктурный "walking skeleton":
пустой `/health`, заглушка-страница на фронте, миграции, CI-воркфлоу.
Деплой на живой VDS в рамках этого этапа **не выполнялся** — см. раздел
"Деплой — TODO" ниже.
...
## MVP-ограничения (этап 01)

- Доменной логики нет — только `/health`, заглушка-страница и миграция-плейсхолдер
  (`schema_version`). Доменные таблицы и сущности появятся в этапе 03.
- Авторизации нет (этап 02 — Telegram Login + JWT).
- Деплой на VDS не выполнялся — см. "Деплой — TODO".
