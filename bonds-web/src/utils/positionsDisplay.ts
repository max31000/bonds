...
/** Задача 24: подсказка про «грязные» цены (с НКД) — у заголовка «Рыночная стоимость» и строки «Итого». */
export const DIRTY_PRICE_TOOLTIP_TEXT =
  'Все стоимости — «грязные»: включают накопленный купонный доход (НКД). ' +
  'Так же считает брокер при продаже/покупке.';
