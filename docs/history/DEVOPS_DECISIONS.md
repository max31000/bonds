...
| Токен T-Invest НЕ заводится как GitHub Secret/ENV вообще | Хранится только в БД на аккаунт, вводится через UI (`PUT /api/settings/tinvest-token`) — деплой-пайплайн и контейнер его не видят |
