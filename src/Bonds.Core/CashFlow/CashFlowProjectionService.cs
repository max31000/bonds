...
public static class CashFlowProjectionService
{
    ...
    public static IReadOnlyList<ProjectedCashFlow> Project(PositionCashFlowInput input)
    {
        ...
        foreach (var item in items)
        {
            ...
            // от типа купона — это та же логика деградации, что у движка этапа 05. DataIncomplete
            // на уровне инструмента (spec §4.4 — MOEX мог вернуть не все купоны) тоже помечает
            // ВСЕ потоки оценочными: пропущенный купон в "дырке" графика отсутствует в Coupons
            // целиком (не присутствует с IsKnown=false), поэтому сам по себе не сработал бы
            // здесь — без этого флага проекция выглядела бы полной там, где платёж может
            // отсутствовать в построенном календаре.
            var isEstimated = isFloaterLike || !item.IsKnown || input.DataIncomplete;
