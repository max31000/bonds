...
public static class MoexBondizationParser
{
    ...
    private static List<OfferSchedule> ParseOffers(JsonElement root)
    {
        ...
        foreach (var row in table.Rows())
        {
            ...
            //
            // ОГРАНИЧЕНИЕ ДАННЫХ (зафиксировано при ревью этапов 04-06): во всех живых фикстурах
            // MOEX ISS, собранных на этапе 04 (tests/Bonds.Tests/Fixtures/Moex/), реально
            // наблюдалось РОВНО ОДНО значение offertype — "Оферта/Погашение" (put). Ни одного
            // реального примера с call в фикстурах нет, поэтому подстрочная эвристика для call
            // НЕ подтверждена реальными данными — она основана на ожидании, что MOEX использует
            // слово "call"/"колл" в этом случае, но это не проверено живым ответом. Дефолт на Put
            // для всего неоднозначного — осознанно консервативный выбор: put-оферта требует
            // активного действия инвестора (иначе бумага просто остаётся в портфеле, spec §8 —
            // "легко пропустить, высокая ценность"), а call — не требует. Ошибочная классификация
            // call-оферты как put в худшем случае создаст лишнее уведомление "проверьте оферту"
            // (ложноположительный сигнал, не критично); обратная ошибка (пропустить реальный put)
            // была бы хуже. Если этап 07 (сигналы) или владелец продукта столкнётся с реальной
            // call-офертой и эвристика её неверно классифицирует — нужно расширить список
            // распознаваемых подстрок по факту нового наблюдения, а не гадать заранее.
