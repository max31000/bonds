...
public class InstrumentRepository : IInstrumentRepository
{
    ...
    private const string SelectColumns = @"
        id AS Id, isin AS Isin, secid AS Secid, figi AS Figi, issuer AS Issuer, sector AS Sector,
    ...
    public async Task<ulong> UpsertAsync(Instrument instrument)
    {
        using var conn = CreateConnection();
        const string sql = @"
            INSERT INTO instruments (isin, secid, figi, issuer, sector, face_value, currency, coupon_type,
                                      has_amortization, has_offers, maturity_date, data_incomplete,
                                      is_out_of_scope_currency)
            VALUES (@Isin, @Secid, @Figi, @Issuer, @Sector, @FaceValue, @Currency, @CouponType,
                    @HasAmortization, @HasOffers, @MaturityDate, @DataIncomplete, @IsOutOfScopeCurrency)
            ON DUPLICATE KEY UPDATE
                secid = @Secid, figi = @Figi, issuer = @Issuer, sector = @Sector,
